...
also see the data coming in, in the Grafana dashboard.
...
to simulate a gap in the data too.
...
it is pretty easy to spot the gap in the data. In both variants. Bust just assume how that would look is the data
would not be such a predictable pattern. You might actually miss if the sensor value is a flat line, or if it was a
gap in the data.

== Better partitioning

Of course, in the background, TimescaleDB knows best how to optimize the data and queries. By transforming the standard
